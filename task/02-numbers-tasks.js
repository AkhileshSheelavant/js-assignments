...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var a=x1*x2;
    var b=y1*y2;
    var c=Math.sqrt(Math.pow(x1,2)+Math.pow(y1,2))*Math.sqrt(Math.pow(x2,2)+Math.pow(y2,2));
...
function roundToPowerOfTen(num, pow) {
    ...
    var st=num.toString();
    var len=st.length;
    if(st.charAt(len-pow)<5){
        num=num-(st.substring(len-pow));
        return num;
    }
    else if(st.charAt(len-pow)>=5){
        num=num+(Math.pow(10,pow)-st.substring(len-pow));
        return num;
    }
//    throw new Error('Not implemented');
}
